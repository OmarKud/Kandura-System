...
<style>
  ...
  .thead,.row{display:grid;grid-template-columns: 1fr 180px 260px;gap:10px;align-items:center;padding:12px 14px}
...
<div class="wrap">
  <div class="hero">
    <h2>Roles</h2>
    <p>Create roles and assign permissions (guard: api).</p>
  </div>
  ...
  <div class="panel">
    <form method="GET" action="{{ route('dashboard.superadmin.roles.index') }}" style="display:flex;gap:10px;flex-wrap:wrap;align-items:end;width:100%">
      <div class="field" style="flex:2;min-width:280px">
        <div class="label">Search</div>
        <input class="input" name="q" value="{{ $q }}" placeholder="Role name">
      </div>
      ...
      <div class="right" style="margin-left:auto">
        ...
        <a class="btnx btnx-ghost" href="{{ route('dashboard.superadmin.roles.index') }}">Reset</a>
        <a class="btnx" href="{{ route('dashboard.superadmin.roles.create') }}">+ Create</a>
      </div>
    </form>
  </div>
  ...
  <div class="table">
    ...
    @forelse($roles as $r)
      <div class="row">
        ...
        <div style="color:var(--muted);font-weight:900">{{ $r->permissions_count }}</div>
        <div class="right">
          <a class="btnx btnx-ghost" href="{{ route('dashboard.superadmin.roles.edit', $r) }}">Edit</a>
          <form method="POST" action="{{ route('dashboard.superadmin.roles.destroy', $r) }}">
            @csrf
            @method('DELETE')
            <button class="btnx btnx-danger" type="submit" onclick="return confirm('Delete this role?')">Delete</button>
          </form>
        </div>
      </div>
    @empty
      <div style="padding:16px;text-align:center;color:var(--muted);font-weight:900">No roles found.</div>
    @endforelse
  </div>
