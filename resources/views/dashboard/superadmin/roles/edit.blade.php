...
<div class="wrap">
  ...
  <div class="card">
    <form method="POST" action="{{ route('dashboard.superadmin.roles.update', $role) }}">
      @csrf
      @method('PUT')
      ...
      <div class="field">
        ...
        <div class="grid">
          @foreach($permissions as $p)
            @php
              $checked = in_array($p->id, old('permissions', $selected ?? []));
            @endphp
            <label class="perm">
              <input type="checkbox" name="permissions[]" value="{{ $p->id }}" {{ $checked ? 'checked' : '' }}>
              <div>
                <b>{{ $p->name }}</b>
